...
<div class="container-xxl">
    <div class="training-card">
        ...
        <div class="info-section">
            ...
            @if ($trainerUsers->count() == 1)
            <p class="trainer-name">المدرب:
                <span>{{ $trainerUsers->first()->getTranslation('name', 'ar') }}</span>
            </p>
            @else
            <p class="trainer-name">المدربون:
                @foreach ($trainerUsers as $trainerUser)
                <span>{{ $trainerUser->getTranslation('name', 'ar') }}</span>
                @if (!$loop->last)
                ,
                @endif
                @endforeach
