                ...
                <div class="left-col">
                    ...
                    <div class="info-message">
                        ...
                        <div class="info-message-content">
                            أضف الإعدادات الإضافية الخاصة بالتدريب مثل تكلفة الدورة، موقع انعقادها، الحد الأقصى للمشاركين، الملفات المرفقة، والصورة التعريفية. تساعد هذه المعلومات في تقديم تجربة واضحة للمتدربين وتنظيم الدورة بشكل فعّال
                        </div>
                    </div>
                    
                    <form id="publish-training-5-form" method="POST" action="{{ route('training.store.settings', $training->id) }}" enctype="multipart/form-data">
                        @csrf
                        
<div class="input-group">
    <label class="switch">
        <span class="switch-label">دورة مجانية</span>
<input type="checkbox" name="is_free" id="is_free" {{ old('is_free', $settings->is_free ?? false) ? 'checked' : '' }}>
...
<div style="width: 100%;">
    @php
        ...
        if (is_numeric($cost)) {
            $formattedCost = ($cost == floor($cost)) ? number_format($cost, 0, '', '') : number_format($cost, 2, '.', '');
        } else {
            $formattedCost = 0;
        }
    @endphp
    <input type="number" step="0.01" name="cost" value="{{ $formattedCost }}" placeholder="مثال: 200" class="@error('cost') error-border @enderror" />
    @error('cost')
    ...
    @enderror
                            ...
                            <div class="input-group mt-4">
                                ...
                                <textarea name="payment_method" rows="5" placeholder="اكتب هنا" class="@error('payment_method') error-border @enderror">{{ old('payment_method', $settings->payment_method ?? '') }}</textarea>
                        ...
                        @unless ($presentation_method_name === TrainingAttendanceType::HYBRID->value || $presentation_method_name === TrainingAttendanceType::REMOTE->value)
                            <div class="input-group location-group">
                                ...
                                <select class="custom-singleselect @error('country_id') error-border @enderror" name="country_id" id="country_id" data-placeholder="اختر الدولة">
                                    <option value="" selected disabled>اختر الدولة</option>
                                    @foreach ($countries as $country)
                                        <option value="{{ $country->id }}" {{ old('country_id', $settings->country_id ?? '') == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                                    @endforeach
                                </select>
                                @error('country_id')
                                    <span class="error-message">{{ $message }}</span>
                                @enderror
                                <select class="custom-singleselect @error('city') error-border @enderror" name="city" id="city" data-placeholder="اختر المدينة" {{ old('city', $settings->city ?? '') ? '' : 'disabled' }}>
                                    <option value="" disabled {{ old('city', $settings->city ?? '') ? '' : 'selected' }}>اختر المدينة</option>
                                </select>
                                @error('city')
                                    <span class="error-message">{{ $message }}</span>
                                @enderror
                                <textarea name="residential_address" rows="2" placeholder="العنوان بالتفصيل" class="@error('residential_address') error-border @enderror">{{ old('residential_address', $settings->residential_address ?? '') }}</textarea>
    ...
    @enderror
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="max_trainees" value="{{ old('max_trainees', $settings->max_trainees ?? '') }}" placeholder="مثال: 20" class="@error('max_trainees') error-border @enderror" />
                            @error('max_trainees')
                                <span class="error-message">{{ $message }}</span>
                            @enderror
                        </div>
...
<div class="input-group">
    <label>ما الطريقة التي تفضلها لاستقبال طلبات المشاركة بالتدريب: <span class="required">*</span></label>
    <div class="sub-label">
    ...
    <div class="radio-group">
        @foreach (\App\Enums\ApplicationSubmissionMethod::cases() as $method)
            <div class="radio-group-item">
                <input type="radio" name="application_submission_method" value="{{ $method->value }}" id="{{ $method->value }}" {{ old('application_submission_method', $submissionMethod) == $method->value ? 'checked' : '' }} class="@error('application_submission_method') error-border @enderror" />
...
<div class="input-group" id="registration-link-section" style="{{ old('application_submission_method', $submissionMethod) === 'outside_platform' ? '' : 'display: none;' }}">
    <label>رابط التسجيل</label>
    <input type="url" name="registration_link" placeholder="الصق هنا" value="{{ old('registration_link', $settings->registration_link ?? '') }}" class="@error('registration_link') error-border @enderror" />
                                ...
                                <div class="profile-image-wrapper">
                                    ...
                                    @else
                                        <div class="profile-image-preview">
                                            <img src="{{ asset('storage/' . $settings->profile_image) }}" alt="صورة التدريب" class="profile-image-shown image-show" />
                                            <input type="hidden" name="existing_profile_image" value="{{ $settings->profile_image }}">
                                            <button type="button" class="profile-image-btn change-profile-image">تغيير الصورة</button>
                                        </div>
                                    @endif
                                    <input type="file" id="profile_image" name="profile_image" accept="image/*" class="visually-hidden @error('profile_image') error-border @enderror">
                                </div>
                                @error('profile_image')
                                    <span class="error-message">{{ $message }}</span>
                                @enderror
...
<div class="input-group">
    ...
    <textarea name="welcome_message" rows="5" placeholder="اكتب رسالة ترحيبية هنا" class="@error('welcome_message') error-border @enderror">{{ old('welcome_message', $settings->welcome_message) }}</textarea>
    ...
    @enderror
                        ...
                        <div class="input-group-2col mt-4">
                            <div class="input-group">
                                <a href="{{ route('training.schedule', $training->id) }}" class="pbtn pbtn-outlined-main">
                                    السابق
                                </a>
                            </div>
                            <div class="input-group">
...
document.getElementById('is_free').addEventListener('change', function() {
    ...
    if (this.checked) {
        ...
        const selectedMethod = document.querySelector('input[name="application_submission_method"]:checked');
