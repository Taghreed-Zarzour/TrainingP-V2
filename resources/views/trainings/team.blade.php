...
@section('title', 'إدارة الفريق - التدريب')
@section('css')
@endsection
@section('content')
    <main>
                ...
                <div class="left-col">
                    ...
                    <div class="info-message">
                        ...
                        <div class="info-message-content">
                            أضف المدرب أو المدربين الذين سيقدمون هذا التدريب، ويمكنك كذلك إضافة مساعد مدرب أو ميسر إن وُجد. تأكد من اختيار الأشخاص المسجلين مسبقًا في المنصة، أو قم بدعوتهم لإكمال بياناتهم. سيتم عرض أسماء المدربين والمساعدين في صفحة التدريب.
                        </div>
                    </div>
                    ...
                    <form id="publish-training-3-form" method="POST" action="{{ route('training.store.team', $training->id) }}">
                        ...
                        <div class="input-group">
                            ...
                            <div class="trainer-main">
                                @if (Auth::user()->photo)
                                    <img src="{{ asset('storage/' . Auth::user()->photo) }}" alt="{{ Auth::user()->name }}" class="custom-rounded" width="120" height="120">
                                @else
                                    <img src="{{ asset('images/icons/user.svg') }}" alt="{{ Auth::user()->name }}" class="custom-rounded" width="120" height="120">
                                @endif
                                <div class="trainer-info">
                                    <div class="trainer-name m-0">{{ Auth::user()->getTranslation('name', 'ar') }} {{ Auth::user()->trainer->getTranslation('last_name', 'ar') }}</div>
                                </div>
                            </div>
                        </div>
                        @endif
                            ...
                            <div class="trainers" name="trainer_ids[]">
                                @foreach ($availableTrainers as $trainer)
                                    @if (in_array($trainer->id, $currentTrainers))
                                        <div class="trainer-main" data-trainer-id="{{ $trainer->id }}">
                                            <button type="button" class="trainer-main-remove" title="حذف">×</button>
                                            @if ($trainer->photo)
                                                <img src="{{ asset('storage/' . $trainer->photo) }}" alt="{{ $trainer->name }}" class="custom-rounded" width="120" height="120">
                                            @else
                                                <img src="{{ asset('images/icons/user.svg') }}" alt="{{ $trainer->name }}" class="custom-rounded" width="120" height="120">
                                            @endif
                                            <div class="trainer-info">
                                                <div class="trainer-name m-0">{{ $trainer->getTranslation('name', 'ar') }} {{ optional($trainer->trainer)->getTranslation('last_name', 'ar') }}</div>
                                                <div class="trainer-email">{{ $trainer->email }}</div>
                                            </div>
                                            <input type="hidden" name="trainer_ids[]" value="{{ $trainer->id }}" />
                                        </div>
                                    @endif
                                @endforeach
                            </div>
                        ...
                        <div class="input-group-2col mt-4">
                            <div class="input-group">
                                <a href="{{ route('training.goals', $training->id) }}" class="pbtn pbtn-outlined-main">
                                    السابق
                                </a>
                            </div>
                            <div class="input-group">
                                <button type="submit" class="pbtn pbtn-main">
                                    حفظ والمتابعة للجدولة
                                </button>
                            </div>
                        </div>
                    </form>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll('.add-more-btn').forEach(function(addMoreBtn) {
                addMoreBtn.addEventListener('click', function() {
                    ...
                    // إذا كان هناك حد أقصى للمدربين/المساعدين
                    if (maxSelection === "1") {
                        // حذف أي مدرب/مساعد موجود مسبقاً
