...
<div class="container py-5">
    ...
    <div class="upload-container" id="dropArea">
        ...
        <form action="{{ route('training.file.upload', $program->id) }}" method="POST" enctype="multipart/form-data" id="uploadForm">
            @csrf
  ...
  <!-- الملفات المرفوعة -->
@if(!empty($program->AdditionalSetting->training_files))
    @php
        // تحويل training_files إلى مصفوفة إذا كان نص JSON
        $trainingFiles = $program->AdditionalSetting->training_files;
        if (is_string($trainingFiles)) {
            $trainingFiles = json_decode($trainingFiles, true) ?? [];
        }
        
        // التأكد من أنها مصفوفة
        if (!is_array($trainingFiles)) {
            $trainingFiles = [];
        }
    @endphp
    ...
    @if(count($trainingFiles) > 0)
        <div class="mt-4">
            ...
            <ul class="list-group mt-3">
                @foreach($trainingFiles as $index => $file)
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <form method="POST" action="{{ route('training.file.delete', ['program_id' => $program->id, 'file_index' => $index]) }}">
                        @csrf
                        @method('DELETE')
