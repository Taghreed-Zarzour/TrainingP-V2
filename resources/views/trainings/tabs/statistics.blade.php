    ...
    <div class="stats-row d-flex flex-wrap gap-3">
        <div class="stat-card">
            ...
            <div class="stat-info">
                ...
                <div class="stat-value">{{ $program->views }} مشاهدة</div>
            </div>
        </div>
        <div class="stat-card">
...
<script>
    ...
    const sessionCount = @json(count($program->sessions));
    const sessionLabels = Array.from({length: sessionCount}, (_, i) => `الجلسة ${i+1}`);
    const sessionAttendanceCounts = @json(array_values($sessionAttendanceCounts));
    // console.log('labels:', sessionLabels);
    // console.log('dataValues:', sessionAttendanceCounts);
</script>
{{-- مكتبة Chart.js --}}
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
    ...
    const dataValues = sessionAttendanceCounts;
    const maxDataValue = Math.max(...dataValues);
    const yAxisMax = Math.ceil(maxDataValue * 2);
    // دالة لرسم نص متعدد الأسطر (تُستخدم في البالون المخصص)
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                borderWidth: 3,
                tension: 0.4,
                pointRadius: (ctx) => ctx.dataIndex === dataValues.indexOf(Math.max(...dataValues)) ?
                    8 : 0,
