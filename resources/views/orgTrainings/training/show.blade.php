...
@section('title', 'تعديل الجلسات التدريبية')
@section('content')
    <!-- Blue Header Section -->
        ...
        <div class="container d-flex justify-content-center">
            <div class="col-12 col-lg-7 text-center">
                <div class="title-wrapper">
                    <h1 class="d-inline-block lh-base">تعديل الجلسات التدريبية</h1>
                </div>
                <div class="mb-4">
                    <div class="mb-4" id="breadcrumb-tab-title">
                        التدريبات/الجلسات التدريبية/تعديل الجلسات
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="container-xxl">
        <div class="training-card">
            ...
            <div class="info-section">
                <h1 class="training-title m-0">تعديل الجلسات التدريبية</h1>
                <p class="trainer-name">يمكنك إضافة أو تعديل أو حذف الجلسات التدريبية لهذا البرنامج</p>
            </div>
            ...
            <div class="container-fluid py-3">
                <ul class="nav simple-tabs justify-content-around mb-4" id="mainTabs" role="tablist"
                    ...
                    <li class="nav-item" role="presentation">
                        <a class="nav-link active" id="sessions-tab" data-bs-toggle="tab" href="#sessions" role="tab"
                ...
                <div class="tab-content" id="mainTabsContent">
                    <div class="tab-pane fade show active px-5" id="sessions" role="tabpanel"
                            ...
                            <div class="tr-sessions-card">
                                ...
                                <div class="tr-sessions-table-container">
                                    <table class="tr-sessions-table">
                                        ...
                                        <tbody>
                                            @foreach ($training->trainingSchedules as $index => $schedule)
                                                <tr data-session-number="{{ $index + 1 }}">
                                                    <td data-label="التاريخ">{{ $schedule->session_date }}</td>
                                                    <td data-label="التوقيت">
                                                        {{ \Carbon\Carbon::parse($schedule->session_start_time)->format('g:i A') }} -
                                                        {{ \Carbon\Carbon::parse($schedule->session_end_time)->format('g:i A') }}
                                                    </td>
                                                    <td data-label="الحضور">{{ $scheduleAttendanceCounts[$schedule->id] ?? 0 }}</td>
                                                    <td data-label="الحالة">
                                                        <span class="tr-status-badge 
                                                            {{-- @if ($sessionStatuses[$schedule->id] == 'مكتمل') tr-status-completed
                                                            @elseif($sessionStatuses[$schedule->id] == 'قيد التقدم') tr-status-in-progress
                                                            @else tr-status-not-started @endif">
                                                            {{ $sessionStatuses[$schedule->id] ?? 'غير محدد' }} --}}
                                                        </span>
                                                    </td>
                                                    <td data-label="المتدربون">
                                                        {{-- <button class="tr-attendees-btn"
                                                            onclick="window.location.href='{{ route('sessions.attendance', $schedule->id) }}?view={{ $sessionStatuses[$schedule->id] == 'مكتمل' ? 'show' : 'edit' }}'"
                                                            @if ($sessionStatuses[$schedule->id] == 'لم تبدأ') disabled @endif>
                                                            @if ($sessionStatuses[$schedule->id] == 'مكتمل')
                                                                عرض الحضور
                                                            @else
                                                                تحديد الحضور
                                                            @endif
                                                    ...
                                                    <td data-label="تفاعل">
                                                        <div class="tr-action-buttons">
                                                            <form class="p-0" action="{{ route('orgSessions.destroy', $schedule->id) }}"
                                                            ...
                                                            <button class="tr-action-btn tr-action-edit" data-bs-toggle="modal"
                                                                data-bs-target="#editSessionModal{{ $schedule->id }}">
                                                ...
                                                <!-- مودال تعديل الجلسة -->
                                                <div class="modal fade" id="editSessionModal{{ $schedule->id }}" tabindex="-1" aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-centered modal-lg">
                                                        <div class="modal-content px-3 py-3">
                                                            ...
                                                            <div class="modal-body">
                                                                <form action="{{ route('orgSessions.update', $schedule->id) }}" method="POST" class="p-0">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    ...
                                                                    <div class="input-group mb-4">
                                                                        ...
                                                                        <div class="row g-3">
                                                                            <div class="col-md-4">
                                                                                <input type="date" name="session_date" class="form-control" value="{{ $schedule->session_date }}" required>
                                                                            </div>
                                                                            <div class="col-md-4">
                                                                                <div class="time-picker-container">
                                                                                    ...
                                                                                    <input type="hidden" name="session_start_time" class="time-picker-input" 
                                                                                           value="{{ \Carbon\Carbon::parse($schedule->session_start_time)->format('H:i') }}">
                                                                                </div>
                                                                            </div>
                                                                            <div class="col-md-4">
                                                                                <div class="time-picker-container">
                                                                                    ...
                                                                                    <input type="hidden" name="session_end_time" class="time-picker-input" 
                                                                                           value="{{ \Carbon\Carbon::parse($schedule->session_end_time)->format('H:i') }}">
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
    ...
    <div class="modal fade" id="addSessionModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-md">
            <div class="modal-content px-3 py-3">
                ...
                <div class="modal-body">
                    <form id="addSessionForm" action="{{ route('organizationSessions.store', $training->id) }}" class="p-0" method="POST">
                        @csrf
                            ...
                            <div class="session-group" style="position: relative; margin-top: 20px;">
                                <div class="input-group">
                                    ...
                                    <div class="input-group-2col inner mb-3" style="align-items: flex-start;">
                                        <div style="width: 100%;">
                                            <input type="date" name="session_date" placeholder="مثال: 15 يونيو 2025" required>
                                        </div>
                                        <div style="width: 100%;">
                                            <div class="time-picker-container">
                                                <div class="time-picker" data-type="start">وقت بدء الجلسة</div>
                                                <div class="time-picker-dropdown"></div>
                                                <input type="hidden" name="session_start_time" class="time-picker-input" value="">
                                            </div>
                                        </div>
                                        <div style="width: 100%;">
                                            <div class="time-picker-container">
                                                ...
                                                <div class="time-picker-dropdown"></div>
                                                <input type="hidden" name="session_end_time" class="time-picker-input" value="">
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('form[action*="orgSessions/update"]').forEach(form => {
