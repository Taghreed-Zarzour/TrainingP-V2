<!-- resources/views/orgTrainings/tabs/registrants.blade.php -->
<style>
    .trainee-card {
    ...
    <div class="row justify-content-center">
        <div class="controls-row col-md-11 d-flex justify-content-lg-between justify-content-center align-items-center flex-wrap gap-2">
            <div class="d-flex align-items-center gap-3">
                ...
                <form method="POST" action="{{ route('participants.bulkAccept', $OrgProgramDetail->trainingProgram->id) }}"
                    ...
                    <div class="card trainee-card position-relative">
                        ...
                        <div class="trainee-content align-content-center">
                            <div class="trainee-info">
                                <img src="{{ $participant->photo ? asset('storage/' . $participant->photo) : asset('images/icons/user.svg') }}"
                                    class="rounded-circle" width="60" height="60">
                                <div class="trainee-details">
                                    <h5 class="mb-1">
                                        {{-- {{ $participant->getTranslation('name', 'ar') }}
                                        {{ $participant->getTranslation('last_name', 'ar') }}</h5> --}}
                                    <p class="text-muted small mb-0">
                                        سجّل بتاريخ
                                        {{ $pivot?->created_at ? \Carbon\Carbon::parse($pivot->created_at)->locale('ar')->translatedFormat('d F Y') : 'غير معروف' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        @if ($isRejected)
                            <div class="trainee-actions">
                                <button class="action-btn reject-reason-btn"
                                    ...
                                    data-program="{{ $OrgProgramDetail->trainingProgram->id }}">
                        ...
                        @else
                            <div class="trainee-actions">
                                <form class="d-flex flex-row w-100 p-0" method="POST"
                                    action="{{ route('participants.handleAction', ['program' => $OrgProgramDetail->trainingProgram->id, 'participant' => $participant->id]) }}">
                                    @csrf
        ...
        buttons.forEach(btn => {
            btn.addEventListener("click", function() {
                ...
                if (view === "list") {
                    ...
                    traineesGrid.classList.remove("row-cols-1", "row-cols-md-2", "row-cols-lg-3", "g-3");
                    ...
                    traineesGrid.classList.add("row-cols-1", "row-cols-md-2", "row-cols-lg-3", "g-3");
...
<script>
    ...
    rejectModal.addEventListener('show.bs.modal', function(event) {
        ...
        const routeTemplate = "{{ route('participants.submitReason', ['program' => 'PROGRAM_ID', 'participant' => 'PARTICIPANT_ID']) }}";
        const newAction = routeTemplate
            .replace('PROGRAM_ID', programId)
            .replace('PARTICIPANT_ID', participantId);
        form.setAttribute('action', newAction);
    });
</script><!-- resources/views/orgTrainings/tabs/registrants.blade.php -->
<style>
    .trainee-card {
        transition: all 0.3s ease;
        border-radius: 22px;
